...
if uploaded_file:
    ...
    if st.button("🔍 စစ်ဆေးမည် (OCR Scan)"):
        with st.spinner(f"{num_cols_active} တိုင်ကို အသေးစိတ် ဖတ်နေပါသည်..."):
            ...
            h, w = img.shape[:2]
            # ၈ တိုင်အစား ရွေးချယ်ထားသော num_cols_active အတိုင်း Grid ဆောက်မည်
            ...
            # ၂။ Dynamic Column Boundaries (ရွေးထားသော တိုင်အရေအတွက်အလိုက် အချိုးချခြင်း)
            # ၆ တိုင်ဆိုလျှင် 1/6 စီ၊ ၈ တိုင်ဆိုလျှင် 1/8 စီ ခွဲဝေပါမည်
            ...
            for (bbox, text, prob) in results:
                ...
                if 0 <= r_idx < num_rows and 0 <= c_idx < num_cols_active:
                    ...
                    # Number Columns (0, 2, 4, 6 တိုင်များအတွက်)
                    if c_idx % 2 == 0:
                        txt = re.sub(r'[^0-9R]', '', txt)
                        if len(txt) > 3 and 'R' not in txt:
                            txt = txt[:3]
                    # Amount Columns (1, 3, 5, 7 တိုင်များအတွက်)
                    else:
                        txt = re.sub(r'[^0-9X*]', '', txt)

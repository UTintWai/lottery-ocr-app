...
if uploaded_file:
    ...
    if st.button("🔍 စစ်ဆေးမည် (OCR Scan)"):
        with st.spinner(f"{num_cols_active} တိုင်ကို အမြင့်ဆုံးတိကျမှုဖြင့် ဖတ်နေပါသည်..."):
            # ၁။ Image Enhancement
            ...
            # OCR Resolution မြှင့်တင်ခြင်း
            results = reader.readtext(processed_img, detail=1, contrast_ths=0.05, adjust_contrast=0.9)

            # ၂။ ၆ တိုင်အတွက် လက်ရေးမူနှင့် အံကိုက် Column သတ်မှတ်ချက်
            # D1-6 အကွက်များ မိစေရန် Column Boundary များကို ညှိထားပါသည်
            if num_cols_active == 6:
                col_steps = [0.18, 0.32, 0.52, 0.65, 0.85, 1.0]
            else:
                col_steps = [0.13, 0.24, 0.38, 0.49, 0.63, 0.74, 0.88, 1.0]

            for (bbox, text, prob) in results:
                # ရှေ့ဆုံးဂဏန်းများ မလွတ်စေရန် left_x (ဘယ်ဘက်အစွန်း) ကို ဦးစားပေးသည်
                left_x = bbox[0][0]
                cx = np.mean([p[0] for p in bbox])
                cy = np.mean([p[1] for p in bbox])
                
                # Column index ရှာဖွေခြင်း
                ...
                if 0 <= r_idx < num_rows and 0 <= c_idx < num_cols_active:
                    ...
                    # Character Mapping (စာလုံးမှ ဂဏန်းသို့)
                    repls = {'S': '5', 'T': '7', 'Z': '7', 'G': '6', 'I': '1', 'L': '1', 'O': '0', 'B': '8', 'A': '4'}
                    for k, v in repls.items():
                        txt = txt.replace(k, v)

                    # Number Columns (0, 2, 4) အတွက် သုံးလုံးဂဏန်း Enforcement
                    if c_idx % 2 == 0:
                        txt = re.sub(r'[^0-9R]', '', txt)
                        # ဂဏန်း ၂ လုံးတည်းဖြစ်နေပါက 0 ဖြည့်မည် (ဥပမာ- 70 -> 070)
            ...
            # ၃။ Ditto Logic (ဒစ်တို သင်္ကေတများ ရှင်းထုတ်ခြင်း)
            for c in range(num_cols_active):
                last_val = ""
                for r in range(num_rows):

...
# ---------------- ၃။ OCR SCAN LOGIC (အကွက်မကျဲစေရန် အချောသတ် version) ----------------
if st.button("🔍 စစ်ဆေးမည် (OCR Scan)"):
    with st.spinner("အကွက်စိပ်စိပ်ဖြင့် ဖတ်နေပါသည်..."):
        ...
        # OCR က ရလာတဲ့ result တွေကို အပေါ်မှအောက်၊ ဘယ်မှညာ အရင်စီလိုက်ပါသည် (အရေးကြီးသည်)
        results = sorted(results, key=lambda x: (x[0][0][1], x[0][0][0]))

        # Column အကွာအဝေးကို အချိုးကျ တွက်ချက်ခြင်း
        step_w = 1.0 / num_cols_active
        col_steps = [step_w * (i + 1) for i in range(num_cols_active)]

        for (bbox, text, prob) in results:
            if prob < 0.2: continue # ယုံကြည်စိတ်ချရမှု နည်းလွန်းလျှင် ကျော်မည်
            
            # စာလုံး၏ ဗဟိုချက် တွက်ခြင်း
            ...
            if 0 <= r_idx < num_rows and 0 <= c_idx < num_cols_active:
                ...
                # အမှားများနိုင်သော စာလုံးများကို ဂဏန်းသို့ ပြောင်းခြင်း
                ...
                # အကယ်၍ အကွက်ထဲမှာ စာရှိနေရင် Space လေးခြားပြီး ပေါင်းပေးပါ (အကွက်မကျော်စေရန်)
                if grid_data[r_idx][c_idx]:
                    grid_data[r_idx][c_idx] += " " + txt
                else:
                    grid_data[r_idx][c_idx] = txt

        # Ditto Logic (အပေါ်က ဂဏန်းအတိုင်း ပြန်ယူခြင်း)
        for c in range(num_cols_active):
            last_val = ""
            for r in range(num_rows):
                curr = grid_data[r][c].strip()
                # Ditto လို့ ယူဆရတဲ့ သင်္ကေတများ

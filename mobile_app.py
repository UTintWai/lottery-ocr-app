...
if uploaded_file:
    ...
    if st.button("🔍 စစ်ဆေးမည် (OCR Scan)"):
        with st.spinner(f"{num_cols_active} တိုင်ကို အသေးစိတ် ဖတ်နေပါသည်..."):
            # ၁။ Image Pre-processing ကို အားကောင်းစေခြင်း
            gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
            # Contrast ကို အမြင့်ဆုံး ညှိထားသည် (အဖျော့စာလုံးများ ပိုထင်ရှားစေရန်)
            ...
            # OCR results ဖတ်ရာတွင် အစွန်းထွက်စာလုံးများ မကျန်စေရန် x_ths နှင့် y_ths ကို လျှော့ချထားသည်
            results = reader.readtext(processed_img, detail=1, contrast_ths=0.05, adjust_contrast=0.8, x_ths=0.1, y_ths=0.1)

            # ၂။ Column အချိုးအစား (Manual Fine-tuning)
            # တိုင်များ ကပ်နေလျှင် ဤတန်ဖိုးများကို အနည်းငယ် တိုး/လျှော့ လုပ်နိုင်သည်
            col_steps = [(i+1)/num_cols_active for i in range(num_cols_active)]

            for (bbox, text, prob) in results:
                # Bounding box ၏ ဘယ်ဘက်အစွန်းကို ပိုဦးစားပေး၍ Column ရှာရန်
                left_x = bbox[0][0] 
                cx = np.mean([p[0] for p in bbox])
                cy = np.mean([p[1] for p in bbox])
                
                # ရှေ့ဆုံးဂဏန်းများ မလွတ်စေရန် left_x နှင့် cx ကို မျှတစွာ သုံးထားသည်
                ...
                if 0 <= r_idx < num_rows and 0 <= c_idx < num_cols_active:
                    ...
                    # အက္ခရာမှ ဂဏန်းသို့ အမှားပြင်ခြင်း
                    repls = {'S': '5', 'T': '7', 'Z': '7', 'G': '6', 'I': '1', 'L': '1', 'O': '0', 'B': '8', 'A': '4'}
                    for k, v in repls.items():
                        txt = txt.replace(k, v)

                    # --- A, C, E, G တိုင်များအတွက် သီးသန့် Logic ---
                    if c_idx % 2 == 0:
                        # ဂဏန်းနှင့် R မဟုတ်တာအားလုံး ဖယ်မည်
                        txt = re.sub(r'[^0-9R]', '', txt)
                        
                        # (၁၂၀ ကို ၂၀ ဖတ်ခြင်းမှ ကာကွယ်ရန်) စာလုံးအရှည်ကို စစ်ဆေးမည်
                        # အကယ်၍ ဂဏန်းက သုံးလုံးထက် ကျော်နေပါက အလယ် သို့မဟုတ် အဓိက အပိုင်းကို ယူမည်
                        if len(txt) > 3 and 'R' not in txt:
                            # ဤနေရာတွင် ရှေ့ဆုံး ၃ လုံးကို တိကျစွာ ယူသည်
                            txt = txt[:3]
                        elif len(txt) < 3 and txt.isdigit():
                            # ဂဏန်း ၂ လုံးတည်း ဖြစ်နေပါက ရှေ့တွင် ၀ ထည့်မည် (ဥပမာ- 70 -> 070)
                            txt = txt.zfill(3)

                    # --- B, D, F, H တိုင်များအတွက် သီးသန့် Logic ---
                    else:
                        txt = re.sub(r'[^0-9X*]', '', txt)

                    # လက်ရှိအကွက်မှာ data ရှိနေပြီးသားဖြစ်ပါက (OCR က စာလုံးတစ်လုံးချင်း ခွဲဖတ်မိလျှင်) ပေါင်းထည့်မည်
                    if grid_data[r_idx][c_idx] == "":
                        grid_data[r_idx][c_idx] = txt
                    else:
                        # ရှေ့ဂဏန်းပြုတ်မသွားစေရန် လက်ရှိဖတ်မိတာကို ရှေ့ကနေ ပေါင်းပေးခြင်း
                        grid_data[r_idx][c_idx] = (txt + grid_data[r_idx][c_idx])[:3] if c_idx % 2 == 0 else txt

            # ၃။ Ditto & Final Cleanup
            for c in range(num_cols_active):
                last_val = ""
                for r in range(num_rows):
                    curr = str(grid_data[r][c]).strip()
                    is_ditto = curr in ['"', "''", "4", "LL", "V", "11", "U", "W", "-"] or (not curr.isalnum() and curr != "")

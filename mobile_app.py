...
@st.cache_resource
def load_ocr():
    # ဖုန်းပုံများအတွက် ပိုမိုတိကျသော mode သုံးထားသည်
    return easyocr.Reader(['en'], gpu=False)

reader = load_ocr()

def scan_voucher_final(img, active_cols, num_rows):
    # ၁။ ပုံကို ကြည်လင်အောင် ပြုပြင်ခြင်း
    gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
    # အလင်းပြန်ခြင်းနှင့် ဝါးခြင်းကို လျှော့ချရန် Contrast မြှင့်သည်
    ...
    for (bbox, text, prob) in results:
        ...
        if 0 <= r < num_rows and 0 <= c < active_cols:
            ...
            # Ditto သင်္ကေတများကို "DITTO" ဟု အမှတ်အသားပြုမည်
            if t in ['"', '||', '။', '..', '\"', '4', 'U']: # OCR မှားတတ်သော Ditto ပုံစံများ
                t = "DITTO"
    ...
    for c in range(active_cols):
        for r in range(1, num_rows):
            # အကယ်၍ လက်ရှိအကွက်က DITTO ဖြစ်နေလျှင် အပေါ်ကွက်ကတန်ဖိုးကို ယူမည်
            if "DITTO" in grid_data[r][c]:
...
with st.sidebar:
    ...
    # ဒေတာပို့မည့် Sheet ကို ဤနေရာတွင် ရွေးပါ
...
if 'sheet_data' in st.session_state:
    ...
    if st.button("🚀 Send to Google Sheet"):
        try:
            ...
            # ဤနေရာတွင် sidebar မှ ရွေးထားသော sheet ကို ဖွင့်ပါမည်
            ...
            if clean_rows:
                ...
                st.success(f"✅ ဒေတာများကို {target_sheet} ထဲသို့ ပို့ပြီးပါပြီ!")
            else:
